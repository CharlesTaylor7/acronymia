...
COPY . .
RUN cargo build --release --package=acronymia --bin=acronymia --target-dir=target/server --features=ssr
RUN cargo build --release --package=acronymia --lib --target-dir=target/front --target=wasm32-unknown-unknown --features=hydrate

FROM chef as bundler
